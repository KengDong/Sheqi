...
# HUMAN PROSE GATE V1.1｜商业正文人类可读性闸门
...
出现**1项严重反复模式**，或出现**多项彼此独立的明显AI模式**：
> 不送作者，先Commercial Comparator返工。

这里不按“命中数量”机械计分。单个轻微痕迹可以WARN；一个贯穿全文的强模式也足以FAIL。
...
A-T1第一次“舍味”场景结束前，读者至少应形成这些**理解结果**：
...
这些是**读者理解结果，不是要求一段话连续讲完7条**。可以分布在叙述、动作、一个真实知识差对白和后续可见结果中；不必全部塞进对白，更不允许为了“七项全覆盖”写成说明书。
...
> 本文件已通过总评审，作为正式商业正文process gate。
...
1. 本Gate用于消灭**基础可读性与明显AI执行问题**，不是把小说机械评分化。
2. HARD Gate的核心是“第一次读者是否真的能跟上”，不是表面满足某个字数/锚点数量。
3. G12 Shelf Test必须比较**功能相近**的成熟正文，不比较文风是否相似。
4. G13只负责拦截作者本不该替AI发现的基础问题，不替代作者taste，也不能用来要求“所有读者都喜欢”。
5. 若Gate规则和具体文本的自然阅读感冲突，优先检查规则是否被机械执行；不得为了过Gate反而增加AI味。
